    ...
    public class SheetsServiceSetupWizard : EditorWindow
    {
        ...
        private void DrawServiceAccountKeyStep()
        {
            ...
            if (_apiSettings != null)
            {
                ...
                {
                    EditorGUILayout.HelpBox($"⚠ ファイルが見つかりません: Assets/StreamingAssets/{_apiSettings.ServiceAccountKeyFileName}", MessageType.Warning);
                }
        ...
        private void DrawSpreadsheetConfigStep()
        {
            ...
            // 新しいスプレッドシート追加
            EditorGUILayout.LabelField("新しいスプレッドシートを追加:", EditorStyles.boldLabel);
            
            _newSpreadsheetName = EditorGUILayout.TextField("名前", _newSpreadsheetName);
            _newSpreadsheetId = EditorGUILayout.TextField("スプレッドシートID", _newSpreadsheetId);
            _newSpreadsheetDescription = EditorGUILayout.TextField("説明（任意）", _newSpreadsheetDescription);
            ...
            // 接続テスト
            if (_serviceInstance != null && configs.Count > 0)
            {
